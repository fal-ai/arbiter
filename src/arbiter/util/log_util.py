...
logger = getLogger("arbiter")


@contextmanager
def debug_logger(log_level: int | str = DEBUG) -> Iterator[Logger]:
    """
...
class ColoredLoggingFormatter(Formatter):
    ...
    def format(self, record: LogRecord) -> str:
        ...
        return {
            "CRITICAL": maybe_use_termcolor(
                formatted, "red", attrs=["reverse", "blink"]
            ),
...
arbiter_static_handlers: list[Handler] = []
arbiter_static_level: int = 99
arbiter_is_frozen: bool = False
...
class FrozenLogger(Logger):
    ...
    def callHandlers(self, record: LogRecord) -> None:
        ...
        """
        global arbiter_static_handlers, arbiter_is_frozen, arbiter_static_level
        from logging import lastResort, raiseExceptions

        c = self
        found = 0
        while c:
            for hdlr in arbiter_static_handlers if arbiter_is_frozen else c.handlers:
                found = found + 1
                if (
                    record.levelno >= arbiter_static_level
                    if arbiter_is_frozen
                    else hdlr.level
                ):
                    hdlr.handle(record)
            if not c.propagate:
...
class UnifiedLoggingContext:
    ...
    @property
    def loggly_handler(self) -> Handler | None:
        """
    ...
    @property
    def fruition_handler(self) -> Handler | None:
        """
    ...
    def get_loggly_handler(self) -> Handler | None:
        """
        A loggly handler, if the environment is configured for it.
        """
        ...
        if token is not None:
            ...
            try:
                ...
                warnings.warn(
                    "Loggly handler requested, but loggly-python-handler not installed."
                )
        return None

    def get_fruition_handler(self) -> Handler | None:
        """
        A fruition handler, if the environment is configured for it.
        """
        url = os.getenv("FRUITION_LOG_URL", None)
        if url is not None:
            tag = os.getenv("FRUITION_LOG_TAG", "arbiter")
            authentication = os.getenv("FRUITION_LOG_AUTH", None)
            interval = float(os.getenv("FRUITION_LOG_INTERVAL", 5))
            kwargs: dict[str, Any] = {"interval": interval, "tag": tag}

            if authentication is not None:
                authentication_parts = authentication.split(":")
                assert (
                    len(authentication_parts) >= 2
                ), "Authentication should of of the form method:username_or_token{:password}"
                method = authentication_parts[0]
                username_or_token = authentication_parts[1]
                password = (
                    authentication_parts[2] if len(authentication_parts) > 2 else None
                )
            ...
            try:
                from fruition.ext.log.handler import (
                    LogAggregateHandler,
    ...
    def start(self) -> None:
        ...
        global arbiter_static_handlers, arbiter_static_level, arbiter_is_frozen
        _acquireLock()
        # First freeze future loggers
        arbiter_is_frozen = True
        arbiter_static_handlers = self.handlers
        if isinstance(self.level, int):
            arbiter_static_level = self.level
        else:
            arbiter_static_level = getLevelName(self.level)
    ...
    def stop(self) -> None:
        ...
        for loggerName, logger in Logger.manager.loggerDict.items():
            if (
                loggerName in self.stored_handlers
                and loggerName in self.stored_levels
                and loggerName in self.stored_propagates
                and isinstance(logger, Logger)
            ):
...
class DebugUnifiedLoggingContext(LevelUnifiedLoggingContext):
    ...
    def __init__(
        ...
        super().__init__(level=DEBUG, silenced=silenced)
