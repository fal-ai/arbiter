...
class DiscoveryMixin:
    ...
    @classmethod
    def include_in_enumeration(cls, introspected_class: type[DiscoveryMixin]) -> bool:
        """
    ...
    @classmethod
    def enumerate(
        cls, introspected_class: type[DiscoveryMixin] | None = None
...
class NamedDiscoveryMixin(DiscoveryMixin):
    ...
    def include_in_enumeration(cls, introspected_class: type[DiscoveryMixin]) -> bool:
        ...
        return getattr(
            cls, "name", None
        ) is not None and super().include_in_enumeration(introspected_class)

    @classmethod
    def get(cls, name: str) -> type[NamedDiscoveryMixin] | None:
        """
        :param name: The name of the class to get.
        :return: The class with the given name, or None if no such class exists.
        """
        for c in cls.enumerate():
            cls_name = getattr(c, "name", None)
            cls_aliases = getattr(c, "aliases", None)
            if cls_name == name or cls_aliases is not None and name in cls_aliases:
