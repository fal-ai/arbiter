...
async def _test_image_nima() -> None:
    with debug_logger():
        async with maybe_download_file(test_image_url, timeout=5.0) as image_path:
            ...
            try:
                ...
                print(f"Skipping NIMA test due to missing deps: {e}")
                return

            score_clean = nima.calculate((image,))
            assert 0.0 <= score_clean <= 1.0 or score_clean > 1.0

            # Add blur degradation
            ...
            score_blurred = nima.calculate((blurred,))

            # Add noise degradation
            noise = torch.randn_like(image) * 0.15
            noisy = torch.clamp(image + noise, 0, 1)
            score_noisy = nima.calculate((noisy,))

            # Expect degradations to generally not improve aesthetic score
            assert score_blurred <= score_clean + 0.1
            assert score_noisy <= score_clean + 0.1


def test_image_nima() -> None:
    asyncio.run(_test_image_nima())
